...
export function loadItemsDefault({commit}){
	setTimeout(() => {
        console.log('@actions!  loadItemsDefault @ funerals!')
...
export function loadItemsLocal({commit},payload){
    setTimeout(() => {
        console.log('@actions!  loadItemsLocal @ weddings! Payload:',payload)
...
export function getSingleItem({commit, state},payload){
  return new Promise((resolve, reject) => {
    setTimeout(() => {
        console.log('@actions!  getSingleItem!',payload)
        const item = state.list.find(current_item => current_item.n_inscricao == payload.n_inscricao)
        console.log("@actions!  getSingleItem! Item found",item)
        return resolve(item);
